...
const DoctorDashboard = () => {
  ...
  const { data: cases = [], isLoading } = useQuery({
    queryKey: ["doctor-cases", doctorRecord?.id],
    queryFn: async () => {
    ...
    <div>
      ...
      <div className="bg-card border border-border rounded-xl overflow-hidden">
        <div className="flex items-center justify-between p-5 border-b border-border">
          <h2 className="font-semibold text-foreground">My Cases</h2>
          <Button variant="ghost" size="sm" onClick={() => navigate("/doctor/cases")} className="gap-1 text-primary">
        ...
        {isLoading ? (
          ...
          <p className="text-muted-foreground p-5 text-center">No doctor profile linked to your account. Contact admin.</p>
        ) : cases.length === 0 ? (
